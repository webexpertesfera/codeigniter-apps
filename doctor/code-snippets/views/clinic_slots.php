...
<style type="text/css">
                ...
                <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    ...
                    <div class="jarviswidget" id="wid-id-5" data-widget-colorbutton="false"
                        ...
                        <header role="heading" class="ui-sortable-handle add-fee-header">
                            ...
                            <h2>Clinic Time Slot</h2>
                        </header>



  <form id="new_doctor_clinic" method="post" class="form-horizontal" action="<?php echo base_url('doctor/doctors/update_clinic_slots');?>" enctype="multipart/form-data">
   
   <input type="hidden" name="clinic_id" value="<?php echo $clinic_id;?>">
    <input type="hidden" name="doctor_id" value="<?php echo $doctor_id;?>">
   <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
    <div class="form-field2 form-group">
      <label><?php echo $this->lang->line('Morning Maximum Visitor'); ?> </label>
      ...
      for($i=1;$i<=20;$i++)
      {
        ...
        if($clinics[0]->maximum_visitor == (5*$i))
        {
          $select = 'selected';
        }
        else
      ...
      ?>
       ...
       <h6 id="morning_allowed" style="color: red;"></h6>
    </div>
  </div>
  <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
    <div class="form-field2 form-group">
      <label><?php echo $this->lang->line('Evening Maximum Visitor'); ?></label>
       <select class="form-control" name="eve_patient_allowed" id="eve_patient_allowed" required>
      <option value=""><?php echo $this->lang->line('Select'); ?></option>
      <?php 
      for($i=1;$i<=20;$i++)
      {
        if($clinics[0]->evening_maximum_visitor == (5*$i))
        {
          $select = 'selected';
        }
        else
      ...
      ?>
        ...
        <h6 id="evening_allowed" style="color: red;"></h6>
    </div>
  </div>
  ...
  <div class="schedule-days">
    ...
    <h5> <input class="messageCheckbox_1" type="checkbox" name="day[]" id="<?php echo $value;?>" value="<?php echo $key;?>" <?php echo $check;?>><?php echo $value;?></h5>
<?php } ?>
  </div>
  <div class="schedule-slots">
    <div class="slots-head">
     <h4> <?php echo $this->lang->line('Morning Slots'); ?> </h4>
   </div>
   <div class="slots-data">
    <?php foreach($time as $key => $value){
     ...
     <div class="row">
      <div>
       <input type="time" name="<?php echo $value['day'].'_morning_from';?>" id="<?php echo $value['day'].'_morning_from';?>" placeholder="<?php echo $this->lang->line('From'); ?>" value="<?php echo $value['start_time'] == '00:00:00'?'':$value['start_time']; ?>" />
     </div>
     <div>
       <input type="time" name="<?php echo $value['day'].'_morning_to';?>" id="<?php echo $value['day'].'_morning_to';?>" placeholder="<?php echo $this->lang->line('To'); ?>" value="<?php echo $value['end_time'] == '00:00:00'?'':$value['end_time']; ?>" />
     </div>
   </div>
 <?php } ?>
</div>
...
<div class="schedule-slots">
  <div class="slots-head">
   <h4><?php echo $this->lang->line('Evening Slots'); ?>  </h4>
 </div>
 <div class="slots-data">  
  <?php foreach($time as $key => $value){
        ?>
   <div class="row">
    <div>
     <input type="time" name="<?php echo $value['day'].'_evening_from';?>" id="<?php echo $value['day'].'_evening_from';?>" placeholder="<?php echo $this->lang->line('From'); ?>"  value="<?php echo $value['evening_start_time'] == '00:00:00'?'':$value['evening_start_time']; ?>"/>
   </div>
   <div>
     <input type="time" name="<?php echo $value['day'].'_evening_to';?>" id="<?php echo $value['day'].'_evening_to';?>" placeholder="<?php echo $this->lang->line('To'); ?>"  value="<?php echo $value['evening_end_time'] == '00:00:00'?'':$value['evening_end_time']; ?>"/>
   </div>
 </div>
  <?php } ?>
</div>
...
<div class="schedule-button">
  <input type="submit" value="submit" type="button" class="btn btn-info" id="checkBtn">
</div>
